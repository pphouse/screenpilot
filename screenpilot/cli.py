...
@click.pass_context
def run(ctx: click.Context, goal: str, max_steps: int, provider: str | None, model: str | None) -> None:
    ...
    def on_step(step: StepResult) -> None:
        ...
        reasoning = step.action.reasoning[:50] + "..." if len(step.action.reasoning) > 50 else step.action.reasoning
        step_table.add_row(
...
def screenshot(ctx: click.Context) -> None:
    ...
    from screenpilot.vision import ScreenCapture, ScreenAnalyzer
...
def record(ctx: click.Context, name: str, description: str) -> None:
    ...
    console.print(Panel(
        ...
        f"Press [bold red]Ctrl+C[/] to stop recording.",
        title="Recorder",
    ))
